...
with open('../raw_data/qg_valid.json') as inpfile:
    ...
    lexrank = LexRank([item['intro'] + ' ' + item['chapter_text'] for item in raw_dataset], stopwords=STOPWORDS['en'])
...
for df in all_df:
    ...
    df['lexrank'] = lexrank.rank_sentences([str(v) for v in df['text'].values.tolist()], threshold=0.1)

    ground_truth = df.sort_values(by='label', ascending=False)['label'].values

    corpus_label = df.sort_values(by='lexrank', ascending=False)['label'].values

    corpus_text = df.sort_values(by='lexrank', ascending=False)['text'].values
